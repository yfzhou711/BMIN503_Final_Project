...
filename = "Venn_Tcell_Dysfunction_5genes.png",
	col = "transparent",
	fill = 1:5,
	alpha = 0.50,
	cex = 1,
...
filename = "Venn_Correlation_DEG.png",
	col = "transparent",
	fill = 2:3,
	alpha = 0.50,
	cex = 1,
...
Data.t.r <- scale(data.combine.r$PDCD1,center = TRUE, scale = TRUE) # z score scaling
test.data.r <- data.frame(data.combine.r[,1:3], Data.t.r)
colnames(test.data.r) <- c('sample', 'OS', 'OS.time', 'PDCD1')
test.data.r$PDCD1 <- ifelse(test.data.r$PDCD1> 1, 'High','Low') # one standard deviation above average expression was defined as "High"

library(survival)
library(survminer)


ggsurvplot(survfit(Surv(OS.time, OS) ~ PDCD1,
                   data = test.data.r),
           data = test.data.r,
           pval = TRUE,
           xlab = 'Month',
