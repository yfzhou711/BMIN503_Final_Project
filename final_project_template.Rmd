...
data.rsem <- read.table(gzfile("HiSeqV2.PADD.RSEM.normalized.gz"), sep="\t", header=TRUE, stringsAsFactors=FALSE)
columnname.r <- str_replace_all(colnames(data.rsem), "[.]" , "-")
colnames(data.rsem) <- columnname.r

# load survival datasets
data.survival <- read.table(gzfile("TCGA-PAAD.survival.tsv.gz"), sep="\t", header=TRUE, stringsAsFactors=FALSE)
...
test.data.r$TNFRSF1B <- ifelse(test.data.r$TNFRSF1B> 1, 'High','Low')
...
```{r}
## 
Survial data will be ranked by overall survival time (os.time), high (75% percentile) and low (25% percentile) os.time samples will be selected for further analysis.
